...
type ControlRequest struct {
	...
	ClientRole  string // the client role as encoded in the client certificate (tcp+tls only)
	UnitId      uint8  // the requested unit id (slave id)
	CodeControl uint8  // code (0x07 or 0x11)
}
...
type RequestHandler interface {
	...
	HandleReadExceptionStatus(req *ControlRequest) (res []uint8, err error)
	HandleReportServerID(req *ControlRequest) (res []uint8, err error)
}
...
func (ms *ModbusServer) handleTransport(t transport, clientAddr string, clientRole string) {
	...
	for {
		...
		case fcReadExceptionStatus:
			status, err := ms.handler.HandleReadExceptionStatus(&ControlRequest{
				ClientAddr:  clientAddr,
				ClientRole:  clientRole,
				UnitId:      req.unitId,
				CodeControl: fcReadExceptionStatus,
			})
			if err != nil {
				break
			}
		...
		case fcReportServerID:
			report, err := ms.handler.HandleReportServerID(&ControlRequest{
				ClientAddr:  clientAddr,
				ClientRole:  clientRole,
				UnitId:      req.unitId,
				CodeControl: fcReportServerID,
			})
			if err != nil {
				break
			}
